...
class ProcessMonitor(QObject):
    ...
    def _emergency_save_all(self):
        ...
        try:
            ...
            # Acquire lock to safely read tracked entries. Use a longer
            # timeout during emergency: losing playtime on shutdown is the
            # very scenario this method exists to prevent.
            acquired = self._data_lock.acquire(timeout=5)
            if not acquired:
                try:
                    logger.warning(
                        "Emergency save: could not acquire data lock within 5s. "
                        "Attempting lockless snapshot of session state."
                    )
                except Exception:
                    pass
                # Best-effort lockless read: _game_sessions is a plain dict
                # mutated only under _data_lock, but at shutdown we accept
                # a torn read rather than losing everything.
                try:
                    sessions_snapshot = {
                        gid: sess["last_save"]
                ...
                for gid, last_save in sessions_snapshot.items():
                    ...
                    if playtime_seconds > 0:
                        try:
                            ...
                            if entry:
                                entry.playtime_seconds += playtime_seconds
                        except Exception:
                            pass
