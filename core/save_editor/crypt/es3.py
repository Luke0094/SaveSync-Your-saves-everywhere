...
# Bundles, which are the same data squeezed — see unityfs. Only reached
...
#
# Kept by game_keys, which every engine with this problem shares.
_KIND = "es3"
...
def _bundle_candidates(path: Path, on_tick=None) -> list:
    ...
    from core.save_editor.crypt.unityfs import SIGNATURE, UnityFsError, unpack
    try:
        raw = path.read_bytes()
    except OSError:
        return []
...
def stored_key(place) -> str:
    ...
    from core.save_editor.crypt.game_keys import stored_key as _stored
